...
void PrintFails()
{
	...
	if (ino::in.Failed())
	{
		...
		ino::out << "\b\b  "; // standard console cant work with backspace character '\b'
#endif
		ino::out << ino::endl;
		ino::in.ClearFails();
	}
...
template <typename T, typename std::enable_if<std::is_same<Custom, typename ino::ReduceTypeExeptConst<T>::type>::value && std::is_lvalue_reference<T>::value, int>::type = 0>
...
void setup() {
	...
	ino::out << ino::Lowercase(string) << ino::endl;
	PrintFails();
	...
	ino::out << ino::Lowercase(CustomData) << ino::endl;
	PrintFails();
